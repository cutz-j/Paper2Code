...
def ShuffleNet(input_shape, scale_factor=1.0, weights=None, classes=40, groups=4, num_shuffle_units=[3, 7, 3], bottleneck_ratio=4, **kwargs):
    '''
    ...
    channel_list = np.asarray(_info(groups), dtype=np.int32)
    channel_list = np.multiply(channel_list, scale_factor)
    print(channel_list)
    
    x = Conv1D(channel_list[0], 3, strides=2, padding='same', use_bias=False, activation=None, name="conv1")(img_input) # 15
    x = BatchNormalization()(x)
    x = Activation('relu')(x)
    
    # stage 2 #
    x = shuffle_unit(x, channel_list[0], out_channels=channel_list[1], groups=groups, strides=2, stage=2, block=1, bottleneck_ratio=bottleneck_ratio)
