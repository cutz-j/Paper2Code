...
from keras.layers import DepthwiseConv2D, Reshape, Dropout, concatenate, ZeroPadding2D, SeparableConv2D
...
def NASNet(input_shape=None, penultimate_filters=768, num_blocks=6, skip_reduction=True,
    ...
    input_shape = _obtain_input_shape(input_shape, default_size=default_size,
                                      min_size=32, data_foramt=K.image_data_format(),
    ...
    # Normal Cell x N
    for i in range(num_blocks):
        x, p = _normal_A(x, p, filters*filter_multiplier, weight_decay, id='%d' %(num_blocks+i+1))
...
def _separable_conv_block(ip, filters, kernel_size=(3, 3), strides=1, block_id=None):
    '''
    ...
    with K.name_scope('separable_conv_block_%s' % block_id):
        ...
        if strides == 2:
            ...
            conv_pad = 'same'
        x = SeparableConv2D(filters, kernel_size, strides=strides, padding=conv_pad, use_bias=False,
    ...
    with K.name_scope('adjust_block'):
        if p is None:
            ...
            with K.name_scope('adjust_reduction_block_%s' % block_id):
                ...
                p1 = AveragePooling2D((1, 1), strides=2, padding='valid', name='adjust_avg_pool_1_%s'%id)(p)
                p1 = Conv2D(filters//2, kernel_size=(1, 1), padding='same', use_bias=False,
                            kernel_regularizer=l2(weights_decay), kernel_initializer='he_normal',
                            name='adjust_conv_1_%s' % block_id)(p1)
                p2 = ZeroPadding2D(padding=((0, 1), (0, 1)))(p)
                p2 = Cropping2D(cropping=((1,0), (1,0)))(p2)
                p2 = AveragePooling2D((1, 1), strides=2, padding='valid', name='adjust_avg_pool_2_%s' %id)(p2)
                p2 = Conv2D(filters//2, kernel_size=(1, 1), padding='same', use_bias=False,
                            kernel_regularizer=l2(weights_decay), kernel_initializer='he_normal',
        ...
        elif p.keras_shape[channel_dim] != filters:
            with K.name_scope('adjust_projection_block_%s'%block_id):
                p = Activation('relu')(p)
                p = Conv2D(filters, kernel_size=(1, 1), padding='same', use_bias=False,
                            kernel_regularizer=l2(weights_decay), kernel_initializer='he_normal',
...
def _normal_A(ip, p, filters, weight_decay=5e-5, block_id=None):
    ...
    with K.name_scope('normal_A_block' %block_id):
        ...
        with K.name_scope('block_1'):
            x1 = _separable_conv_block(h, filters, kernel_size=(3, 3), weight_decay=weight_decay,
                                       block_id='normal_left1_%s'%block_id)
            x1 = Add([x1, h], name='normal_add_1_%s'%block_id)
        with K.name_scope('block_2'):
            x2_1 = _separable_conv_block(p, filters, kernel_size=(3, 3), weight_decay=weight_decay,
            ...
            x2 = Add([x2_1, x2_2], name='normal_add_1_%s'%block_id)
        ...
        with K.name_scope('block_4'):
            x4_1 = AveragePooling2D((3, 3), strides=1, padding='same', nmae='normal_left4_%s'%(block_id))(p)
            x4_2 = AveragePooling2D((3, 3), strides=1, padding='same', nmae='normal_right4_%s'%(block_id))(p)
        ...
        with K.name_scope('block_5'):
            ...
            x5 = Add([x5_1, x5_2], name='normal_add_5_%s'%block_id)
...
def _reduction_A(ip, p, filters, weight_decay=5e-5, block_id=None):
    ...
    with K.name_scope('reduction_A_block_%s' % block_id):
        ...
        # 우측 부터
        with K.name_scope('block_5'):
            x5_1 = AveragePooling2D((3, 3), strides=1, padding='same', name='reduction_left5_%s'%block_id)(h)
            x5_2 = _separable_conv_block(p, filters, (5, 5), strides=2, weight_decay=weight_decay,
                                         block_id='reduction_right5_%s'%block_id)
            x5 = Add([x5_1, x5_2], name='reduction_add5_%s'%block_id)
        with K.name_scope('block_4'):
            ...
            x4 = Add([x4_1, x4_2], name='reduction_add_4_%s'%block_id)
        with K.name_scope('block_2'):
        ...
        with K.name_scope('block_3'):
            ...
            x3 = Add([x3_1, x4], name='reduction_add_3_%s'%block_id)
        with K.name_scope('block_1'):
