...
class ShakeFunction(Function):
    """
    # Function: shake function forward / bacward
    """
...
def get_alpha_beta(batch_size, shake_config, device):
    """
    # Function: alpah / beta for shake-shake droppath
    # Arguments:
        batch_size
        shake_config: forward / bacward shake strategy (Even/Shake | Even/Shake/Keep)
        device
    """
...
def initialize_weights(module):
    """
    # Function: Weight/bias initialization (He_Normal)
    """
...
class DownsamplingShortcut(nn.Module):
    ...
    def forward(self, x):
        ...
        y2 = F.pad(x[:,:,1:,1:], (0, 1, 0, 1)) # 1 pixel shift
...
class BasicBlock(nn.Module):
    """
    # Function: ShakeShake basic module / right
    # Arguments: in, out, stride, shake_config
    """
    ...
    def forward(self, x):
        ...
        alpha, beta = get_alpha_beta(batch_size=x.size(0), shake_config=shake_config, device=x.device)
